...
def interpolate_pos_embed(model, checkpoint_model, orig_size=(6,10), new_size=(3,10)): 
    ...
    if 'decoder_pos_embed' in checkpoint_model:
        ...
        decoder_embedding_size = decoder_pos_embed_checkpoint.shape[-1] # 512
        ...
        if orig_size != new_size:
            ...
            # only the position tokens are interpolated
            decoder_pos_tokens = decoder_pos_embed_checkpoint[:, num_extra_tokens:] # old positions
            decoder_pos_tokens = decoder_pos_tokens.reshape(-1, orig_size[0], orig_size[1], decoder_embedding_size).permute(0, 3, 1, 2)
            decoder_pos_tokens = torch.nn.functional.interpolate(
                decoder_pos_tokens, size=(new_size[0], new_size[1]), mode='bicubic', align_corners=False)
            pos_tokens = decoder_pos_tokens.permute(0, 2, 3, 1).flatten(1, 2)
            new_decoder_pos_embed = torch.cat((extra_tokens, pos_tokens), dim=1)
            checkpoint_model['decoder_pos_embed'] = new_decoder_pos_embed
...
#        print("pos_embed_checkpoint.shape:", pos_embed_checkpoint.shape)   #[1, 61, 768]
