...
def get_args_parser():
    ...
    parser.add_argument('--time_mask_ratio', default=0.75, type=float,
                        help='Masking ratio (percentage of removed patches).')    
    parser.add_argument('--masking_scheme', default="random_imputation", type=str,
                        help='masking scheme for pre-training')    
    ...
    parser.add_argument('--pre_mix_up', action='store_true',
                        help='use off-line augmented data') # changed - added
    parser.set_defaults(pre_mix_up=False) # changed - added
    parser.add_argument('--mix_up', action='store_true',
                        help='use on-line augmented data') # changed - added
    parser.set_defaults(mix_up=False) # changed - added

    parser.add_argument('--normalization', action='store_true',
                        help='train and test data set normalization') #
...
def main(args):
    ...
    print(f"n_samples_train = {len(dataset_train.X)},input_size: {args.input_size}, patch_size:{args.patch_size}, patch_num: {args.patch_num}")
    ...
    if args.log_dir is not None:   #changed - global_rank == 0 and ommitted
    ...
    model = models_mae.__dict__[args.model](img_size=args.input_size, patch_size=args.patch_size, in_chans=args.in_chans, # changed - to reflect input_size change
                                            norm_pix_loss=args.norm_pix_loss, alt=args.alt, loss_type=args.loss_type,
                                            masking_scheme=args.masking_scheme,
                                            mask_ratio = args.mask_ratio, var_mask_ratio=args.var_mask_ratio, time_mask_ratio=args.time_mask_ratio)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.output_dir and (epoch % args.dump_freq == 0 or epoch + 1 == args.epochs): # changed - adjusted save_model frequency
            misc.save_model(
