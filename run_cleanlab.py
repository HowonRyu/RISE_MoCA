...
def get_label_issues(SEED, test_data, data_path):
    ...
    df_full = pd.read_csv(os.path.join(data_path, f"pred_gt_df_{split}.csv"), index_col=0)
    ...
    save_npz(os.path.join(data_path, 'pred_gt_df', f'knn_graph_{split}.npz'), knn_graph)
    ...
    label_issues.to_csv(os.path.join(data_path, f"pred_gt_df/cleanlab_df_{split}.csv"), index=False)
...
if __name__ == "__main__":
    ...
    get_label_issues(SEED=args.SEED, test_data=args.test_data, data_path=args.data_path)
